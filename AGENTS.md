...
- 同一任务重试前必须清理残留 `git worktree` 注册，任务失败/取消/重启恢复后要自动回收 workspace，成功任务要保留 worktree 供提交
...
- 内置：`/start`、`/repos`、`/task`、`/status`、`/logs`、`/cancel`、`/submit`、`/clear`、`/reset`
- 插件：`/codex`、`/claude`
- 启动时需要通过 `setMyCommands()` 注册命令菜单
- `/status` 输出应包含当前已选仓库和活跃任务的 worktree 信息
- Codex 任务默认只回传最终结果，不回传中间过程；成功任务完成后需要返回 `task_id`、分支名、worktree 路径
