...
5. 运行链路：仓库选择、`git worktree` 创建、残留 worktree 重试清理、成功任务 worktree 保留、Git/非 Git 发布边界、`/logs` / `/cancel` 默认目标、`/submit` 提交分支与 commit message 语法、`/merge` 合并本地 `main`、`/push` 推送与清理 worktree、最近可执行任务自动选择、按钮顺序与确认交互、`/clear` / `/reset` 行为是否正确
...
8. 文档语义：`/repos` 的 Git-only 行为与直接子目录扫描边界、`workspace::prompt` 和默认路径回退、Codex 最终回包与 `/logs` 回退、发布流默认目标与清理语义、`data/message-history.json` 与会话态持久化边界是否写清楚
